...
interface MobileHeaderProps extends TableHeaderProps {
  ...
  headerGroups: HeaderGroup<ReimbursementRequest>[];
}
...
const MobileTableHeader: React.FC<MobileHeaderProps> = ({
  ...
  const { filters } = useAppSelector((state) => state.financeDashboardState);
  ...
  const handleStatusToggleChange = (e: ButtonGroupOption) => {
    ...
    dispatch(
      setFinanceDashboardFilters({
        ...filters,
        request_status__id: e.value.toString(),
      }),
    );
  };
  return (
